...
def create_sessions_table(con):
    ...
    con.execute(f"""
        ...
        SELECT
            ...
            episode,
            media_type,
            arg_max(app, ts) FILTER (WHERE app IS NOT NULL) as app,
            arg_max(user_name, ts) FILTER (WHERE user_name IS NOT NULL) as user_name,
            arg_max(address, ts) FILTER (WHERE address IS NOT NULL) as address,
            arg_max(device_model, ts) FILTER (WHERE device_model IS NOT NULL) as device_model,
