...
APPLE_TV_ID = "0C5811E9-2D76-4567-B7D5-A7B7A4221562"
...
async def log_now_playing(loop):
    con = init_duckdb()

    storage = FileStorage.default_storage(loop)
    await storage.load()

    atvs = await pyatv.scan(loop, identifier=APPLE_TV_ID, storage=storage)

    if not atvs:
        print("No Apple TV found with that identifier")
        con.close()
        return
    ...
    try:
        ...
        summary_parts = [f"[{ts.isoformat()}]", f"state={state}"]
        if app:
        ...
        print("Authentication error – try re-pairing via atvremote.")
    except Exception as exc:
        print(f"Error while logging now playing: {exc}")
    finally:
        atv.close()
        con.close()
...
if __name__ == "__main__":
    ...
    loop.run_until_complete(log_now_playing(loop))
